...
class Project:
	def startAll(self):
		self.n1 = redis.Redis(port=6179,host='ip',db=0)
		self.n2 = redis.Redis(port=6279,host='ip',db=0)
		self.n3 = redis.Redis(port=6379,host='ip',db=0)
		self.rTime = 0
	...
	def TrueCasing(self,l):
		...
		while line:
			...
			(sentence,time) = trueCaseDistributed(line,self.map,self.n1,self.n2,self.n3)
			output.write(sentence+'\n') 
			self.rTime += time
			#print 'After writing, redis time is ,', self.rTime			
			line = fil.readline()
		output.close()
		print 'time is ',self.rTime			
		self.count+=1
